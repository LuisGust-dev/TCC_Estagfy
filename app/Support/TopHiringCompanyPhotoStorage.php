...
class TopHiringCompanyPhotoStorage
{
    public static function store(UploadedFile $file): string
    {
        if (! static::isSupabaseConfigured()) {
            return $file->store('top-hiring-companies', 'public');
        }

        $extension = $file->getClientOriginalExtension() ?: $file->extension() ?: 'bin';
        $path = 'top-hiring-companies/' . Str::uuid() . '.' . strtolower($extension);
        ...
        if (! $response->successful()) {
            throw new RuntimeException('Falha ao enviar imagem da empresa destaque para o Supabase.');
        }
    ...
    public static function publicUrl(?string $path): ?string
    {
        ...
        if (static::isSupabaseConfigured() && ! str_starts_with($path, 'top-hiring-companies/')) {
